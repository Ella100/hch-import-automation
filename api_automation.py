...
class HCHAPIAutomation:
    ...
    def __init__(self, config_path: str = None, user_role: str = "submitter", environment: str = "qa"):
        ...
        self.headers = {
            ...
            "Authorization": f"Bearer {clean_token(self.token)}",
    ...
    def switch_user(self, user_role: str):
        ...
        if user_role == 'approver' and hasattr(self, 'approver_token') and self.approver_token:
            ...
            self.user_role = user_role
            self.headers["Authorization"] = f"Bearer {clean_token(self.token)}"
            self.session.headers.update(self.headers)
...
def main():
    ...
    plan_type = os.environ.get('PLAN_TYPE', '21').strip()  # 新增计划类型参数 (21=家用, 22=商用, 23=订单机)
    ...
    print(f"选择的计划类型: {plan_type} ({'家用计划' if plan_type == '21' else '商用计划' if plan_type == '22' else '商用订单机'})")
    if submitter_token:
    ...
    if choice == "2":
        # 月需求导入流程
        run_month_demand_flow(submitter_token, approver_token, excel_file_path, environment, plan_type)
    elif choice == "delay":
        # 顺延计划导入流程
        run_month_delay_flow(submitter_token, approver_token, excel_file_path, environment)
    else:
        # 任务单导入流程
        run_task_order_flow(submitter_token, approver_token, excel_file_path, environment, plan_type)


def run_task_order_flow(submitter_token=None, approver_token=None, excel_file_path=None, environment='qa', plan_type='21'):
    """任务单导入流程"""
    from datetime import datetime
    ...
    if submitter_token:
        ...
        api.headers["Authorization"] = f"Bearer {clean_token(submitter_token)}"
    ...
    # 继续后续流程，根据计划类型确定planType参数
    # 21=家用计划 -> planType="21,22", 22=商用计划 -> planType="22", 23=商用订单机 -> planType="23"
    if plan_type == '21':
        api_plan_type = "21,22"
    elif plan_type == '22':
        api_plan_type = "22"
        ...
        api_plan_type = "21,22"
    return run_common_flow(api, import_result, plan_type=api_plan_type, after_time=before_import_time)
...
def run_month_demand_flow(submitter_token=None, approver_token=None, excel_file_path=None, environment='qa', plan_type='21'):
    """月需求导入流程"""
    from datetime import datetime
    ...
    # 继续后续流程，根据计划类型确定planType参数
    # 21=家用计划 -> planType="1,12", 22=商用计划 -> planType="12", 23=商用订单机 -> planType="12"
    if plan_type == '21':
        api_plan_type = "1,12"
    elif plan_type == '22':
        api_plan_type = "12"
